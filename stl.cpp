...
//push back and emplace back both add element in vector but syntax are diffrent
    vec.push_back({1, 8});
    vec.emplace_back(1, 2);
//vec(5 ,100) create vector of 5 index with 100 as element
    ...
    // 10,20,12,23,35
    v.erase(v.begin() + 2, v.begin() + 4); // 10,20,35 it will delete it like  [start,end)
...
void explainPQ()
{
    // Priority Queue greter value element at the top
...
void explainSet()
{
    // set has two features 1.ordered 2. unique element
    set<int> st;
    st.insert(1);  // 1
    st.emplace(2); // 1,2
    st.insert(2);  // 1,2,2
    ...
    auto it1 = st.find(2);
    auto it2 = st.find(4);
    st.erase(it1, it2); // after erase{1,4,5} [first,end) it does not include last element
...
void explainMap()
{
    // maps has keys and value 
    //keys are unique can be think like they are roll number
    // value are like name of the student 
    map<int, int> mpp1;// here key is integer and value is integer
    map<int, pair<int, int>> mpp2;//key is integer value is pair of integer
    map<pair<int, int>, int> mpp3;//key is pair of intege and value is integer
    //maps store unique key in sorted order
